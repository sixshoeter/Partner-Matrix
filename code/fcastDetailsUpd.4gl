...
## May     25, 2017 John M. Shoemaker, jshoemaker1@cox.net - allow for archiving #233
...
define
   ...
   fcast_id_use like fcast_au.fcast_id,
   fcast_cmt like fcast_comments_au.fcast_comment,
   fcast_type like fcast_comments_au.fcast_comment_type,
   updType char(1)
...
#let InRec.ceid = vStr
#display "cmdStr: ", cmdStr clipped, "<br>"
...
#display "kStr: ", kStr clipped, "<br>", "vStr: ", vStr, "<br>"
       ...
       call findIdx( wStr ) returning kStr, vStr, wStr
       let fcastRec.updperson = vStr clipped
...
#         call findIdx( wStr ) returning kStr, vStr, wStr
#         let fcastRec.oppty_stat = vStr clipped
...
#         let fcastRec.industry = vStr clipped
	 ...
	 let fcastRec.brand_grp = vStr clipped
         ...
         call findIdx( wStr ) returning kStr, vStr, wStr
         let fcastRec.brand2 = vStr clipped

         call findIdx( wStr ) returning kStr, vStr, wStr
         let fcastRec.brand3 = vStr clipped

         call findIdx( wStr ) returning kStr, vStr, wStr
         let fcastRec.prod_seg = vStr clipped

         call findIdx( wStr ) returning kStr, vStr, wStr
         let fcastRec.prod_seg2 = vStr clipped

         call findIdx( wStr ) returning kStr, vStr, wStr
         let fcastRec.prod_seg3 = vStr clipped

         call findIdx( wStr ) returning kStr, vStr, wStr
         let fcastRec.prod_fam = vStr clipped

         call findIdx( wStr ) returning kStr, vStr, wStr
         let fcastRec.prod_fam2 = vStr clipped

         call findIdx( wStr ) returning kStr, vStr, wStr
         let fcastRec.prod_fam3 = vStr clipped
         ...
         call findIdx( wStr ) returning kStr, vStr, wStr
         let fcastRec.exp_close_dt = CleanUp(vStr) clipped
         ...
         call findIdx( wStr ) returning kStr, vStr, wStr
         let fcastRec.pass_on = vStr clipped

         call findIdx( wStr ) returning kStr, vStr, wStr
         let fcastRec.ftype = vStr clipped

         call findIdx( wStr ) returning kStr, vStr, wStr
         let fcast_type = vStr clipped

         call findIdx( wStr ) returning kStr, vStr, wStr
         let fcast_cmt = vStr clipped
...
#         let UPDStr = "begin work; delete from ", CtlRec.fcast_tab clipped, " where ssr='", CleanUp(fcastRec.ssr) clipped, "';"
         ...
         if( fcastRec.cust_name is not NULL )then
            if( fcastRec.fcast_id=0 )then
              let updType="I"
              let UPDStr = UPDStr clipped, ' insert into ', 'fcast_au', ' values (', 
                   0, ', ',
              '"', fcastRec.sc clipped, '", ',
	      '"', CleanUp(fcastRec.cust_name) clipped, '", ',
              ...
              '"', CleanUp(fcastRec.brand2) clipped, '", ',
              '"', CleanUp(fcastRec.brand3) clipped, '", ',
              '"', CleanUp(fcastRec.prod_seg) clipped, '", ',
              '"', CleanUp(fcastRec.prod_seg2) clipped, '", ',
              '"', CleanUp(fcastRec.prod_seg3) clipped, '", ',
              '"', CleanUp(fcastRec.prod_fam) clipped, '", ',
              '"', CleanUp(fcastRec.prod_fam2) clipped, '", ',
              '"', CleanUp(fcastRec.prod_fam3) clipped, '", ',
              ...
              '"', CleanUp(fcastRec.pass_on) clipped, '", ',
              '"', CleanUp(fcastRec.ftype) clipped, '", ',
              '"', CleanUp(fcastRec.updperson) clipped, '", ',
              'current', ' ',
#	      '"', CleanUp(fcastRec.oppty_stat) clipped, '" ',
              '); '
            else
              let updType="U"
              let UPDStr = UPDStr clipped, ' update ', 'fcast_au', ' set ',
#              ' oppty_stat="', fcastRec.oppty_stat clipped, '", ',
	      ...
	      ' cust_name="', CleanUp(fcastRec.cust_name) clipped, '", ',
              ...
              'brand2="', CleanUp(fcastRec.brand2) clipped, '", ',
              'brand3="', CleanUp(fcastRec.brand3) clipped, '", ',
              'prod_seg="', CleanUp(fcastRec.prod_seg) clipped, '", ',
              'prod_seg2="', CleanUp(fcastRec.prod_seg2) clipped, '", ',
              'prod_seg3="', CleanUp(fcastRec.prod_seg3) clipped, '", ',
              'prod_fam="', CleanUp(fcastRec.prod_fam) clipped, '", ',
              'prod_fam2="', CleanUp(fcastRec.prod_fam2) clipped, '", ',
              'prod_fam3="', CleanUp(fcastRec.prod_fam3) clipped, '", ',
              ...
              'pass_on="', CleanUp(fcastRec.pass_on) clipped, '", ',
              'ftype="', CleanUp(fcastRec.ftype) clipped, '", ',
              'updperson="', CleanUp(fcastRec.updperson) clipped, '", ',
              'updtime=current ',
              ' where fcast_id=', fcastRec.fcast_id, '; '

               let fcast_id_use = fcastRec.fcast_id
...
if( InRec.form != "975" )then
   ...
   if( SQLCA.sqlcode = 0 )then
      ...
      if( SQLCA.sqlcode = 0 )then
#################################################################
#        get new serial number and insert comment if there is one
#################################################################
         if( updType="I" )then
            if( sqlca.sqlerrd[2] > 0 )then
               let fcast_id_use = sqlca.sqlerrd[2]
            end if  #sqlerrd[2]>0
         end if  #updType="I"
#################################################################
#         display "fcast_id: ", fcast_id_use
#         display "fcast_cmt: ", fcast_cmt
         if( fcast_cmt != " " and fcast_id_use > 0 )then
            let UPDStr = ' insert into fcast_comments_au values(', fcast_id_use,',',
                                         '"', fcast_type clipped, '","', CleanUp(fcast_cmt) clipped, 
                                         '", "', CleanUp(fcastRec.updperson) clipped, '", current)'
#            display UPDStr
            prepare UPD2ID from UpdStr
            execute UPD2ID
         end if  #fcast_cmt not blank
...
#               let runStr = "/home/informix/shoe/cgi-bin/fcastDetailNext", BRec.b_id clipped, ".ksh "
               let runStr = MasterRec.home_dir clipped, "/cgi-bin/fcastDetailNext", BRec.b_id clipped, ".ksh "
#               display runStr
               RUN runStr
            else
