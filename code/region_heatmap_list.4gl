...
let Reg = arg_val(6)  #only the first 5 values are read via ReadCmdLn
...
display "<div dir='ltr' style='text-align: center'>"
display "<em><span style='font-size: 20px'>", RegStr clipped, "/", EUSStr clipped, " Partners with ", t_long clipped, " Skills", "</span></em></div>"
display "<p>"

## Load ALL Data (all brands
let cnt = LoadMasterCovArr( inBrand, Reg, SkiCat )
#display "cnt: ", cnt
...
function LoadMasterCovArr( inBrand, inEUS, inSkillCat )
define
   inBrand like brands.b_id,
   inEUS like eus.e_id,
...
let cnt = GroupCEIDs( inBrand, inEUS, inSkillCat )
#display "cnt: ", cnt

let SQLStr = "select a.ceid from ", cov_tab clipped, " a where a.reseller !='N' and a.reseller !='V' and a.reseller != 'X' and a.part_type!='D' and a.", tmpcolname clipped, " in ('Y','H') and a.ceid in (select ceid from _ceids) into temp st1;"
...
function GroupCEIDs(in_brand, in_eus, in_skillcat )
   define
...
##    determine the column name for the correct industry
      let SQLStr = "select a.colname from _eucolnamesu a where a.idx = '", in_EUS clipped, "'"
      ...
      if( tabStr is not NULL )then
         let SQLStr = "select a.ceid from ", cov_tab clipped, " a ", ", ", tabStr clipped, " where a.ceid = b.ceid and a.reseller != 'N' and a.reseller != 'V' and a.part_type!='D' ",
                   " and (a.", cname clipped, " ='Y' or a.", cname clipped, " ='H') ", 
