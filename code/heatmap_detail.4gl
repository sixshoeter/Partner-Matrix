...
## February 02, 2017 John M. Shoemaker, expand industry beyond only Y values, #204
...
call ReadCmdLn("heatmap_detail", 5)
let Ind = arg_val(5)  #only the first 5 values are read via ReadCmdLn
...
display "<div dir='ltr' style='text-align: center'>"
display "<em><span style='font-size: 20px'>", Ind clipped, " Partner List", "</span></em></div>"
display "<p>"

## Load ALL Data (all brands
let cnt = LoadMasterCovArr( Ind )
...
function LoadMasterCovArr( inInd )
define
...
let SQLStr = "select a.ceid from ", cov_tab clipped, " a where a.reseller !='N' and a.reseller !='V' and a.reseller != 'X' and a.part_type!='D' and a.", tmpcolname clipped, " is not NULL and a.", tmpcolname clipped, " != ' ' into temp st1;"
