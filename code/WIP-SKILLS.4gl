...
## December 04, 2013 John M. Shoemaker, use SkillsMax; alternate product set background
...
define
   skill_cat smallint,
   toggle_flg smallint,
   old_catStr like skill_cat.c_long
...
let s1 = "<td style='text-align: center; background-color: rgb(230,230,250); width: 111px; vertical-align: middle'>"
let s2 = " "
if( CtlRec.skill_cat and skill_cat > 0 )then
   let catStr = GetSkillCatDesc( SkillArr[in_sort].c_id ) 
   let s2 = "<h2><span style='font-size: 16px'>", catStr clipped, "</span></h2>"

   if( (catStr != old_catStr) AND (old_catStr != "<none>") )then
#      if( toggle_flg )then
#         let toggle_flg = FALSE
   let s1 = "<td style='text-align: center; background-color: rgb(255,255,255); width: 111px; vertical-align: middle'>"
#      else
#         let toggle_flg = TRUE
#      end if
   end if
   let old_catStr = catStr
end if
#display "toggle_flg:", toggle_flg
display s1 clipped
if( s2 != " " )then 
   display s2 clipped
end if
