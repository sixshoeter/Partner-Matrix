...
{
   when 'SU'
      let SQLStr = "select unique a.rag, a.nlrev_01, c.*, k.*, l.*, m.*, n.* from tbptv a, svp b, ", cov_tab clipped, " c, skills_s1 k, skills_s2 l, skills_s3 m, skills_s4 n, skills_s5 o where a.rag = b.bptv_name and b.s_brand='", BRec.b_id clipped, "' and year(a.revdt)=year(current) and a.ceid = c.ceid and a.ceid = k.ceid and a.ceid = l.ceid and a.ceid = m.ceid and a.ceid = n.ceid and a.ceid = o.ceid and c.reseller!='N' order by c.part_name"
   when 'CU'
   when 'BU'
      let SQLStr = "select unique a.rag, a.nlrev_01, c.*, k.*from tbptv a, svp b, ", cov_tab clipped, " c, skills_co k where a.rag = b.bptv_name and b.s_brand='", BRec.b_id clipped, "' and year(a.revdt)=year(current) and a.ceid = c.ceid and a.ceid = k.ceid and c.reseller!='N' order by c.part_name"
   otherwise
   let SQLStr = "select c.*, l.* from ", cov_tab clipped, " c, skills_s2 l where c.ceid = l.ceid and c.reseller!='N' order by c.part_name"
end case
}
